...
import { fetchPhongKhamByID, fetchDoctorByPhongKham, getTimeSlotsByDoctorAndDate } from "../../../services/apiDoctor"
...
const ViewPhongKhamVaKhamBenh = () => {

    let location = useLocation();
    // Phân tích cú pháp query string
    let params = new URLSearchParams(location.search);
    const idPhongKham = params.get("idPhongKham"); // Lấy giá trị của tham số "id"
    console.log("check idPhongKham: ", idPhongKham);

    const navigate = useNavigate()
    const [dataPhongKhamByID, setDataPhongKhamByID] = useState(null)
    const [dataDoctorByPhongKham, setDataDoctorByPhongKham] = useState([])
    ...
    useEffect(() => {
        fetchCKByID();
        fetchDoctorByCK(idPhongKham);
    }, [idPhongKham]);

    const fetchCKByID = async () => {

        const res = await fetchPhongKhamByID(idPhongKham)
        console.log("res: ", res);
        if(res && res.data) {
            setDataPhongKhamByID(res.data)
        }
    }

    const fetchDoctorByCK = async () => {

        const res = await fetchDoctorByPhongKham(idPhongKham)
        console.log("res: ", res);
        if(res && res.data) {
            setDataDoctorByPhongKham(res.data)
        }
    }
    ...
    return (
        ...
        <Row>
            <Col span={18} className="col-body" >
                <Row 
                style={{ 
                    backgroundImage: `linear-gradient(rgba(255, 255, 255, 0.8), rgba(255, 255, 255, 0.8)), url(${import.meta.env.VITE_BACKEND_URL}/uploads/${dataPhongKhamByID?.image})`, 
                ...
                >
                    ...
                    <Col span={24}>
                        <span className="title-lichhen"> {dataPhongKhamByID?.name}</span>
                    </Col>   
                    <Col span={24}>
                        <span style={{marginLeft: "15px"}}> 
                            <div style={{marginTop: "-25px", marginLeft: "10px"}} dangerouslySetInnerHTML={{ __html: dataPhongKhamByID?.description }} />
                        </span>
                    </Col>                                                           
                </Row>
            </Col>
            ...
            <Col span={24} className="view-body-xem-lich-ck">
                <Row>
                    <Col span={18} className="col-body" style={{backgroundColor: "rgb(247, 246, 246)", padding: "20px 10px"}} >
                        ...
                        {dataDoctorByPhongKham?.length > 0 ? (
                            dataDoctorByPhongKham.map((item, index) => (
...
export default ViewPhongKhamVaKhamBenh